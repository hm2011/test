...
gulp.task('updateComponentCSS', function(){
    return gulp.src('./static/.tmp/script/module/private/**/*.css',{ base: './static/.tmp/' })
        ...
        .pipe(sourcemaps.write('./map',{
            sourceMappingURLPrefix: config.staticBaseUrl.replace(/\/$/, '') //结尾不能有"/"
        }))
        .pipe(gulpif(typeAssert('css', true), gulp.dest('./static/.tmp/')))
        .pipe(gulpif(function(file){ return file.extname.toLowerCase() === '.map' }, gulp.dest('./static/')));
});
...
gulp.task('buildComponent',gulp.series('prepareComponentResources', 'mergeComponentResources',function(){
    return gulp.src('./static/.tmp/script/module/private/*.js',{base: './static/.tmp/script/'})
        ...
        .pipe(gulpif(typeAssert('js', true), rev()))
        .pipe(gulpif(typeAssert('js', true), revFormat(revFormatOptions)))
...
gulp.task('cssRev',function(){
    return gulp.src('./static/.tmp/style/**/*.css',{ base: './static/.tmp/' })
        ...
        .pipe(cleanCSS({compatibility: 'ie8'}))
        .pipe(sourcemaps.write('./map'))
        .pipe(gulpif(typeAssert('css', true), rev()))
        .pipe(gulpif(typeAssert('css', true), revFormat(revFormatOptions)))
        .pipe(gulp.dest('./static/'))
...
gulp.task('watch', function(cb){
    ...
    gulp.watch(
        [
            ...
            './src/static/script/**/*.tpl',
            './src/static/script/**/*.scss',
            './src/static/script/**/*.png',
            './src/static/script/**/*.gif',
            './src/static/script/**/*.jpg'
        ], 
        gulp.series(
            'script',
            'view', 
...
gulp.task('default', gulp.series(gulp.parallel('script','data','style'),'view'));
